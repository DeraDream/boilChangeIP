...
管理员也可以主动选择 `生成用户` 创建 SS 用户。主动创建时可以选择给管理员本人创建、创建不绑定 TG 的用户，或手动输入指定 TG ID。随后 Bot 会依次询问创建普通 SS 还是 SS2022、端口、密码和加密方式；加密方式会按所选类型列出序号供选择。全部填写完成后才展示确认参数，确认前仍可修改到期日、到期禁用和月流量等参数。
