...
Boil Change IP 是一个用于管理 IPPanel 换 IP 的 Telegram Bot。它可以通过官方 API 查看当前公网 IP、执行换 IP，并生成当前 IP 的质量检测图片。
...
- 通过 IPPanel 官方 Token API 查看当前公网 IP。
- 点击 Telegram Bot 按钮立即执行换 IP。
- 通过 IPPanel 官方 `/api/v1/changeIP/` 接口执行换 IP。
- 生成当前 IP 质量检测图片，并发送到 Telegram。
- 提供 VPS 全局管理命令 `boiltg`。
- 支持版本号更新，更新时保留用户 `.env` 配置。
- 支持 sing-box Shadowsocks 用户管理，每个用户独立端口，支持 SS2022 和普通 SS 加密方式。
...
- `Bot 状态`：查看 Bot 是否运行、当前版本、授权用户、IPPanel API Token 配置状态、SS 用户数、整体入站/出站/出站计费流量，以及每个用户的入站/出站/出站计费流量。
- `获取列表/更换 IP`：显示设备名称和当前 IP，点击设备按钮后立即换 IP。
- `获取当前 IP 质量`：运行双栈完整 IP 质量检测脚本，生成 PNG 图片并发送到 Telegram。

换 IP 后 Bot 会轮询 IPPanel 当前 IP，确认新 IP 后继续轮询域名 A 记录，直到解析到新 IP 再通知 DDNS-GO 已刷新。解析确认域名优先读取 `.env` 中的 `DDNS_DOMAIN`，未配置时使用 `SS_PUBLIC_HOST`。
...
- 到期日：默认当前日期一个月后，可修改
- 到期禁用：默认开启，可关闭
- 月流量：默认 100GB 出站计费，可修改

到期禁用开启时，用户到期后会自动停用。到期禁用关闭时，用户不会停用，到期日会按月向后滚动，并从新的周期重新统计月出站计费流量。
...
管理员也可以主动选择 `生成用户` 创建 SS 用户。主动创建时可以选择给管理员本人创建、创建不绑定 TG 的用户，或手动输入指定 TG ID。随后 Bot 会依次询问创建普通 SS 还是 SS2022、端口、密码、加密方式、有效期、流量额度和是否每整一个月重置流量；加密方式会按所选类型列出序号供选择。有效期可选永久、按月或按日，按月会按日历月计算，例如 7 月 4 日 23:00 创建、1 个月后到期就是 8 月 4 日 23:00。全部填写完成后才展示确认参数，确认前仍可修改到期日、到期禁用和流量等参数。
普通 SS 加密方式跟随 sing-box 支持列表：`none`、`aes-128-gcm`、`aes-192-gcm`、`aes-256-gcm`、`chacha20-ietf-poly1305`、`xchacha20-ietf-poly1305`。SS2022 支持：`2022-blake3-aes-128-gcm`、`2022-blake3-aes-256-gcm`、`2022-blake3-chacha20-poly1305`。
Telegram 创建流程中端口和密码发送 `0` 或 `random` 表示随机。用户流量会持久化保存到 SQLite，避免防火墙规则刷新或服务重启后显示归零。

已授权用户不再使用固定底部键盘，可发送 `/my_ss` 查看链接。

`/my_ss` 会返回 SS 链接、到期日、到期禁用状态、月出站计费流量和状态。Telegram 对 `ss://` 这类自定义协议支持不稳定，Bot 会展示“解析 SS”链接、服务器、端口、加密、密码和一份可复制的原始 SS 链接。
...
流量计费口径为：只统计出站流量，入站流量只展示，不参与额度消耗。

当用户本周期出站计费流量达到月流量额度时，脚本会自动停用该用户，并重载 `sing-box-boil`。
